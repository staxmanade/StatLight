...
namespace StatLight.Core.WebBrowser
{
    internal class WebBrowserFactory
    {
        ...
        public IWebBrowser Create(WebBrowserType browserType, Uri pageToHost, bool browserVisible, bool forceBrowserStart, bool isStartingMultipleInstances, string fileOnDiskPath, Uri uriToXapToHostXap)
        {
            switch (browserType)
            {
                ...
                case WebBrowserType.OutOfBrowser:
                    return new OutOfBrowser(_logger, uriToXapToHostXap, fileOnDiskPath, forceBrowserStart, isStartingMultipleInstances);
            }
